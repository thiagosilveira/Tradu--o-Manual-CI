...
CodeIgniter usa uma versão modificada do padrão da Active Record Database.
Este padrão permite que a informação seja recuperada, inserida e alterada
no seu banco de dados com o mínimo de código. Em alguns casos é necessário
apenas uma ou duas linhas de código para efetuar alguma ação no banco de
dados.
O CodeIgniter não requer que cada tabela tenha a sua própria classe. Ao 
contrário prove uma interface mais simples.

Além da simplicidade, o maior benefício ao usar a Active Record é que a
mesma permite criar aplicações livres do banco de dados uma vez que a
sintaxe é gerada pelo driver do banco de dados. Isso permite também
querys seguras já que os valores são protegidos pelo sistema.

.. nota:: Se você pretende escrever as suas próprias querys, você pode
    desabilitar esta classe no arquivo de configuração database, permitindo
    que as classe de core e o adaptador usem menos recursos.
...
As seguintes funções permitem você construir instruções SQL do tipo **SELECT**

$this->db->get()
================

Executa a query de seleção e retorna os resultados. Pode ser utilizado para
retornar todos os dados de uma tabela::

	$query = $this->db->get('mytable');  // Produz: SELECT * FROM mytable

O segundo e terceiro parâmetro permitem você a fazer limit e offset::

	$query = $this->db->get('mytable', 10, 20);
	// Produz: SELECT * FROM mytable LIMIT 20, 10 (no MySQL. Outros bancos de dados têm uma sintaxe ligeiramente diferente)

Você irá perceber que a função abaixo é guardada na variável $query, que pode
ser usada para exibir os resultados::
...
Visite a página :doc:`result functions <results>` para uma descrição
completa de como retornar resultados.
